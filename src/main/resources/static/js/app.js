...
async function loadStats() {
    ...
    try {
        ...
        const d = await api(isAdmin ? '/api/v1/admin/stats' : '/api/v1/jobs/health');
        if (!d?.data) return;
...
async function loadAdminMetrics() {
    ...
    if (!container || !getApiKey()) return;
    try {
        const d = await api('/api/v1/admin/metrics');
        const metrics = d?.data ?? {};
        container.innerHTML = Object.entries(metrics).map(([name, value]) => `
            <div class="metric-item">
                <div class="label">${escHtml(name)}</div>
                <div class="value">${Number(value ?? 0).toFixed(0)}</div>
            </div>
        `).join('');
    } catch (_) {
        container.innerHTML = '<div class="empty">Unable to load metrics</div>';
    }
}
...
async function loadJobs() {
    ...
    try {
        const d = await api('/api/v1/jobs?size=50');
...
async function submitJob() {
    ...
    try {
        ...
        await api('/api/v1/jobs', { method: 'POST', body: JSON.stringify(reqBody) });
...
async function retryDlqJob(id) {
    try {
        await api(`/api/v1/admin/dlq/${id}/retry`, { method: 'POST' });
...
async function clearQueues() {
    ...
    try {
        await api('/api/v1/admin/queues', { method: 'DELETE' });
...
async function loadDlq() {
    ...
    if (!tbody || !getApiKey()) return;
    try {
        const d = await api('/api/v1/admin/dlq');
        const jobs = d?.data ?? [];
        if (!jobs.length) {
...
async function loadKeys() {
    ...
    if (!tbody || !getApiKey()) return;
    try {
        const d = await api('/api/v1/admin/keys');
        const keys = d?.data ?? [];
        if (!keys.length) {
...
async function loadHandlers() {
    try {
        const d = await api('/api/v1/jobs/types');
...
async function revokeKey(id) {
    ...
    try {
        await api(`/api/v1/admin/keys/${id}`, { method: 'DELETE' });
...
function connectSse() {
    ...
    _sse = new EventSource('/api/v1/events?apiKey=' + encodeURIComponent(getApiKey()));
...
async function syncSecurity() {
    if (!getApiKey()) return;
    try {
        const res = await fetch('/api/v1/admin/stats', { headers: { 'X-API-KEY': getApiKey() } });
        if (res.ok) {
...
async function performLogin() {
    ...
    try {
        const res = await fetch('/api/v1/jobs/health', { headers: { 'X-API-KEY': key } });
        if (res.ok) {
            setApiKey(key);
...
function toggleLive() {
    if (_sse) {
        ...
        _sse = null;
        const el = document.getElementById('sse-label');
        if (el) el.textContent = 'Live Monitoring: Paused';
        return;
    }
    connectSse();
}
