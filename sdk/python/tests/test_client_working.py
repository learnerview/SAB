"""
Test cases for SabClient - Working version with proper async mocking.
"""
...
class TestSabClient:
    ...
    async def test_submit_job_error(self, client):
        ...
        with pytest.raises(SABError, match="Bad request"):
    ...
    async def test_list_jobs_success(self, client):
        ...
        mock_response.json = AsyncMock(return_value={
            "success": True,
            "data": [
                {"jobId": "job-123", "jobType": "webhook", "priority": "NORMAL", "status": "QUEUED"},
                {"jobId": "job-456", "jobType": "webhook", "priority": "HIGH", "status": "RUNNING"}
            ]
        })
    ...
    async def test_wait_for_job_success(self, client):
        ...
        running_response.json = AsyncMock(return_value={
            "success": True,
            "data": {"jobId": "job-123", "jobType": "webhook", "priority": "NORMAL", "status": "RUNNING"}
        })
        
        completed_response = MagicMock()
        completed_response.status = 200
        completed_response.content_type = "application/json"
        completed_response.json = AsyncMock(return_value={
            "success": True,
            "data": {"jobId": "job-123", "jobType": "webhook", "priority": "NORMAL", "status": "SUCCESS"}
        })
